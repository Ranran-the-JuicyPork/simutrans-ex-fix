...
bool scenario_t::load_script(const char* filename)
{
	...
	sprintf( basefile, "%sscript/scenario_base.nut", env_t::base_dir );
...
void scenario_t::load_compatibility_script()
{
	...
	if (api_version != "*") {
		...
		cbuffer_t buf;
		buf.printf("%sscript/scenario_compat.nut", env_t::base_dir );
		...
		if (file->is_loading()) {
			...
			if (env_t::networkmode   &&  !env_t::server) {
				...
				if (env_t::default_settings.get_with_private_paks()) {
					scenario_path = ( std::string("addons/") + env_t::pak_name + "scenario/" + scenario_name.c_str() + "/").c_str();
				...
				if (rdwr_error) {
					scenario_path = (env_t::pak_dir + env_t::pak_name + "scenario/" + scenario_name.c_str() + "/").c_str();
