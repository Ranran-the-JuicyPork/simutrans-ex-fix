...
void nwc_chat_t::rdwr()
{
	...
	dbg->message("nwc_chat_t::rdwr", "rdwr message=%s plnr=%d chnr=%d clientname=%s destination=%s pos=(%d,%d)", message.c_str(), player_nr, channel_nr, clientname.c_str(), destination.c_str(), pos.x, pos.y);
}
