		...
		case NR_INIT:
			...
			if(root==NULL) {
				...
				FOR(vector_tpl<fabrik_t*>, const fab, welt->get_fab_list()) {
					...
					if(  fab->get_desc()->is_consumer_only()  &&  !fab->get_stat(0, FAB_PRODUCTION)  ) {
		...
		case NR_BAUE_SIMPLE_SCHIENEN_ROUTE:
			...
			if(is_connected(start->get_pos().get_2d(), ziel->get_pos().get_2d(), freight)) {
				...
				if(count_rail>=3) {
					...
					create_rail_transport_vehicle( platz1, platz2, count_rail, 100 );
...
DBG_MESSAGE("ai_goods_t::step()","remove already constructed rail between %i,%i and %i,%i and try road",platz1.x,platz1.y,platz2.x,platz2.y);
		...
		case NR_BAUE_STRASSEN_ROUTE:
			...
			if(is_connected(start->get_pos().get_2d(), ziel->get_pos().get_2d(), freight)) {
				...
				create_road_transport_vehicle(start, count_road );
		...
		case CHECK_CONVOI:
		{
			...
			for (size_t i = welt->convoys().get_count(); i-- != 0;) {
				...
				bool delete_this = (finance->get_account_overdrawn() > 0)  &&  (gewinn < -cnv->calc_sale_value());
