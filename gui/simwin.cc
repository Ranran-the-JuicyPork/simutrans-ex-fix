		...
		simwin_gadget_flags_t &flags )
{
	...
	if (sticky) {
		display_proportional_bold_clip(pos.x + (REVERSE_GADGETS?width+4:4)+1, pos.y+(D_TITLEBAR_HEIGHT-LINESPACE)/2+1, text, ALIGN_LEFT, title_color, false);
	}
	int titlewidth = display_proportional_bold_clip( pos.x + (REVERSE_GADGETS?width+4:4), pos.y+(D_TITLEBAR_HEIGHT-LINESPACE)/2+1, text, ALIGN_LEFT, text_color, false );
	if(  flags.gotopos  ) {
		display_proportional_bold_clip( pos.x + (REVERSE_GADGETS?width+4:4)+titlewidth+8, pos.y+(D_TITLEBAR_HEIGHT-LINESPACE)/2+1, welt_pos.get_2d().get_fullstr(), ALIGN_LEFT, text_color, false );
	}
	POP_CLIP();
}
